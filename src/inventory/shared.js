...
export const crudOptions = (baseUrl) => ({
    statics: {
        async getAll() { return await this.find({}).exec(); },
        async create(data) {
            const newObj = new this(data);
            await newObj.save();
            return newObj;
        },
        async getOne(id) { return await this.findById(id).exec(); },
        async replace(id, data) {
            const replacement = new this(data);
            return await this.findOneAndReplace({ _id: id }, replacement, { returnDocument: "after", upsert: true }).exec();
        },
        async remove(id) {
